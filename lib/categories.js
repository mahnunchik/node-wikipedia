...
var getAll = module.exports.all = function(category, opts, callback) {    
	...
	dial(params, function(batch) {
		...
		if (batch.continue && batch.continue.cmcontinue) {
			log.info("Moving on to " + batch.continue.cmcontinue);
			opts.continue = batch.continue.cmcontinue;

			opts.each(batch.query.categorymembers, { terminal: false });
			getAll(category, opts, callback);
		} else if (batch.continue && batch.continue.cmstart) {
			log.info("Moving on to " + batch.continue.cmstart);
			opts.since = batch.continue.cmstart;
